...
# Server Alias: 
Alternate names for a host used when matching requests to name-virtual hosts. Most people simply use ServerName to set the 'main' address of the website (eg. 'mywebsite.com') and ServerAlias to add additional addresses the website will be bound to (eg. 'www.mywebsite.com').

# What is a web server? Hardware and software side

A web server is basically a computer that hosts a website on the Internet. The term web server can refer to hardware or software, or both of them working together.
...
It’s almost synonymous to the World Wide Web, and still powers the majority of websites in the world.The reason for Apache’s dominance is threefold: an open license, early entry (this thing was released way back in 1995!), and easy deployment of PHP. The last point was made possible through the mod_php module, which meant that installing Apache was all you needed to do for developing with PHP. Apache is great because it is available on all platforms – Linux, Windows, MacOS, and other platforms, it’s the default server for all CPanel shared hosting, making it effortless to set up and change sites, it supports for HTTP/2, compression, static files, and load balancing, MPM and FastCGI modes for delivering high concurrency, easy scripting through Lua.
# Nginx
...
SSH, also known as Secure Shell or Secure Socket Shell, is a network protocol that gives users, particularly system administrators, a secure way to access a computer over an unsecured network. SSH also refers to the suite of utilities that implement the SSH protocol. Secure Shell provides strong password authentication and public key authentication, as well as encrypted data communications between two computers connecting over an open network, such as the internet.SSH refers both to the cryptographic network protocol and to the suite of utilities that implement that protocol. SSH uses the client-server model, connecting a Secure Shell client application, which is the end where the session is displayed, with an SSH server, which is the end where the session runs. SSH implementations often include support for application protocols used for terminal emulation or file transfers. SSH can also be used to create secure tunnels for other application protocols, for example, to securely run X Window System graphical sessions remotely. An SSH server, by default, listens on the standard Transmission Control Protocol (TCP) port 22.
 ...
 To create my website, these are the steps I had to follow:
 
 1. Downloading Ubuntu Server
 2. Installing Apache2
 3. configure the firewall
 4. Testing if Apache is running
 5. Fix network Settings
 6. Fixing IP address for the server
 7. Setting up SSH
    ![png](setting%20up%20ssh%20pt.1.png)
    ![png](setting%20up%20ssh%20pt.2.png)
    ![png](setting%20up%20ssh%20pt.3.png)
    ![png](setting%20up%20ssh%20pt.4..png)
    ![png](setting%20up%20ssh%20pt.5..png)

8. Virtual Hosts
   ![png](virtual%20hosts%201.1.png)
   ![png](virtualhosts1.2.png)
   ![png](virtual%20hosts%201.3.png)

9. Creating website
    ![png](creating%20website.png)
    ![png](creating%20website%201.1.png)
    ![png](creating%20website%201.2.png)

10. The project
    ![png](the%20website.png)
    ![png](the%20website%201.1.png)
